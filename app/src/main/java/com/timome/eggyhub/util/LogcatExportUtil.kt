...
object LogcatExportUtil {
     ...
     * 不限制日志行数，直接流式写入文件
     * @param config 用户明确授权的数据收集配置，所有选项默认为false，需用户手动勾选
     */
                ...
                if (!anyLogcatTypeSelected) {
                    ...
                    try {
                        ...
                        while (reader.readLine().also { line = it } != null) {
                            line?.let { logLine ->
                                val logLevel = getLogLevel(logLine)
                                val shouldInclude = when (logLevel) {
                                    'E' -> config.logcatTypes["错误"] == true
                                    'W' -> config.logcatTypes["警告"] == true
                                    'I' -> config.logcatTypes["信息"] == true
                                    'D' -> config.logcatTypes["调试"] == true
                                    'V' -> config.logcatTypes["详细"] == true
                                    else -> false
                                }
     ...
     */
    @RequiresApi(Build.VERSION_CODES.P)
    fun exportLogToSafUri(context: Context, uri: Uri, config: DataCollectionConfig): Boolean {
        return try {
            val dateFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault())
    ...
    /**
     * 从logcat行中提取日志级别
     * logcat格式: "12-31 23:59:59.999  1234  5678 E/TAG: message"
     */
    private fun getLogLevel(logLine: String): Char {
        val parts = logLine.split(" ", limit = 6)
        if (parts.size >= 6) {
            val levelPart = parts[4]
            if (levelPart.isNotEmpty()) {
                return levelPart[0]
            }
        }
        return ' '
