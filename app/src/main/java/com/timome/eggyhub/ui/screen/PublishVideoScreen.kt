...
fun PublishVideoScreen(
    ...
    fun validateBvId(bvId: String): String {
        if (bvId.isBlank()) {
            return "BV号不能为空"
        }
        if (bvId.length != 12) {
        ...
        val prefix = bvId.substring(0, 2)
        if (!prefix.equals("BV", ignoreCase = true)) {
            return "BV号必须以BV开头"
        }
        val suffix = bvId.substring(2)
        for (c in suffix) {
        ...
        return when (currentStep) {
            ...
            2 -> {
                if (bvId.isBlank()) {
                    ...
                    val bvResult = validateBvId(bvId)
                    if (bvResult != "1") {
                        bvError = bvResult
                        false
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    StepProgressBar(
                        ...
                        stepLabels = listOf("名称", "BV号", "描述", "封面"),
                        ...
                        2 -> {
                            OutlinedTextField(
                                ...
                                onValueChange = {
                                    ...
                                    if (bvError != null) bvError = null
                                },
                                label = { Text("BV号") },
                                singleLine = true,
                                modifier = Modifier.fillMaxWidth(),
                                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Text),
                                isError = bvError != null,
                                supportingText = {
                                    bvError?.let {
                                        Text(text = it, color = MaterialTheme.colorScheme.error)
                                    }
